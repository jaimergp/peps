...
The chosen identifier MUST be used in the mapping filenames mentioned above so they can be
...
by a ``+`` symbol. For ecosystems corresponding to Linux distributions,
...
specification. When available, the second field MUST be the ``VERSION_ID`` string.

Some examples:

- Fedora 40: ``fedora+40.json``.
- Ubuntu 24.04: ``ubuntu+24.04.json``.
- Homebrew: ``homebrew.json``
- VCPkg: ``vcpkg.json``
- conda-forge: ``conda-forge.json``
